...
the board cannot have a certain run size or if the parameters are out of 
bounds. */
game* new_game(unsigned int run, unsigned int hangtime, unsigned int width,
               unsigned int height, enum type type)
{
    if (height > 62 || width > 62)
    {
        fprintf(stderr, "new_game: board size out of range\n");
        exit(1);
    }
    ...
    if (run < 2)
    {
        fprintf(stderr, "new_game: run cannot be less than two\n");
        exit(1);
    }
...
bool place_piece(game* g, pos p)
{
    ...
    {
        printf("That spot is off the board\n");
        return false;
    }
    if (board_get(g->b, p) != EMPTY)
    {
        printf("A piece is already in that spot\n");
        return false;
    }
...
/* if_full_board : A helper function that determines if a board is full. */
bool if_full_board(game* g)
{
