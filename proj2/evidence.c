...
unsigned int adjusted_ascii_index(char a)
{
    ...
    {
        fprintf(stderr, "adjusted_ascii_index: index given not supported\n");
        exit(1);
    }
}
...
void evidence_board_new()
{
    ...
    printf("(now with bits board testing)\n");
    // test1 should return an 65x3 empty bits board
    board* test1 = board_new(65, 3, BITS);
    board_show(test1);
    board_free(test1);
    // test2 should return an 5x15 empty bits board
    board* test2 = board_new(5, 15, BITS);
    board_show(test2);
    board_free(test2);
    // test 3 should return a 5x5 empty bits board
    board* test3 = board_new(5, 5, BITS);
    board_show(test3);
    board_free(test3);
    // test 4 should return a 62x62 empty bits board
    board* test4 = board_new(62, 62, BITS);
    board_show(test4);
    board_free(test4);
}
...
void evidence_board_get()
{
    ...
    cell_show(t2_cell2);
    board_free(test2);
    // test3 checks the bits board and should return EMPTY 
    board* test3 = board_new(3, 3, BITS);
    cell t3_cell = board_get(test3, make_pos(2, 2));
    cell_show(t3_cell);
    board_free(test3);
    // test4 checks the bits board and should return WHITE and BLACK 
    board* test4 = board_new(3, 3, BITS);
    board_set(test4, make_pos(2, 2), WHITE);
    board_set(test4, make_pos(0, 0), BLACK);
    cell t4_cell1 = board_get(test4, make_pos(2, 2));
    cell t4_cell2 = board_get(test4, make_pos(0, 0));
    cell_show(t4_cell1);
    cell_show(t4_cell2);
    board_free(test4);
}
...
void evidence_board_set()
{
    ...
    printf("(now with bits board testing)\n");
    /* test1 should return a 3x3 BITS board with the right corner set to 
    WHITE and the top left corner set to BLACK. */
    board* test1 = board_new(3, 3, BITS);
    board_set(test1, make_pos(2, 2), WHITE);
    board_set(test1, make_pos(0, 0), BLACK);
    board_show(test1);
    board_free(test1);
    /* test2 should return a 3x3 bits board with the right corner set to WHITE
...
void evidence_new_game()
{
    ...
    game* test1 = new_game(3, 2, 8, 4, BITS);
    board_show(test1->b);
...
void evidence_place_piece()
{
    ...
    printf("----TEST2----\n");
    game* test2 = new_game(3, 3, 5, 3, BITS);
    printf("--turn 0--\n");
    board_show(test2->b);
    // should return true
    printf("--turn 1--\n");
...
void evidence_game_outcome()
{
    ...
    game_free(test1);
    /* test 2 creates a game of size 3x5 with a run of 3 and a hangtime of 3.
    ...
    printf("----TEST3----\n");
    game* test3 = new_game(2, 2, 8, 4, BITS);
    ...
    printf("----TEST4----\n");
    game* test4 = new_game(2, 0, 8, 4, BITS);
    ...
    game_free(test6);
    /* test7 creates a game of size 20x62 with a run of 2 and a hangtime of 6
    It tests for BLACK_WIN diagonally from NW to SE. */
    printf("----TEST7----\n");
    game* test7 = new_game(2, 6, 62, 20, BITS);
    place_piece(test7, make_pos(10, 61));
    place_piece(test7, make_pos(19, 19));
    place_piece(test7, make_pos(0, 0));
    place_piece(test7, make_pos(2, 0));
    place_piece(test7, make_pos(1, 1));
    board_show(test7->b);
    outcome_interpreter(game_outcome(test7)); // should return BLACK_WIN
    game_free(test7);
}

/* evidence_adjusted_ascii_index : Tests adjusted_ascii_index. */
void evidence_adjusted_ascii_index()
{
    printf("==evidence_adjusted_ascii_index==\n");
    printf("(copied into evidence from main for testing)\n");
    // should return 0
    printf("%d\n", adjusted_ascii_index('0'));
    // should return 7
    printf("%d\n", adjusted_ascii_index('7'));
    // should return 9
    printf("%d\n", adjusted_ascii_index('9'));
    // should return 10
    printf("%d\n", adjusted_ascii_index('A'));
    // should return 19
    printf("%d\n", adjusted_ascii_index('J'));
    // should return 36
    printf("%d\n", adjusted_ascii_index('a'));
    // should return 61
    printf("%d\n", adjusted_ascii_index('z'));
}
